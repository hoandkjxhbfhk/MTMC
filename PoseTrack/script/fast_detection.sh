...
for scene_num in "${SCENES_TO_PROCESS[@]}"; do
    ...
    # Run detection for the specific scene in the background
    # Use --trt for accelerated detection
    taskset -c $cpu_start-$cpu_end python detection/get_detection.py --scene "$scene_num" --ckpt ckpt_weight/bytetrack_x_mot17.pth --trt &
